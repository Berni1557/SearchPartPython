...
Im=cv2.imread('/home/bernifoellmer/Studium/SearchPartPython/SearchPartPython/SearchPartPython/SearchPartPython/data/K1.JPG')
...
#img_name='/home/bernifoellmer/Studium/SearchPartPython/SearchPartPython/SearchPartPython/SearchPartPython/data/Iq.JPG'

imagecv = cv.fromarray(image)

imagecv8 = cv.CreateImage(cv.GetSize(imagecv), cv.IPL_DEPTH_8U, 1)
cv.ConvertScale(imagecv,imagecv8)
#h,w = image.shape
#dst= cv.CreateMat(h, w, cv2.cv.CV_8U)
#image = cv.fromarray(image)

#I=cv.CreateImage( cv.GetSize(imagecv), cv.CV_8U, imagecv.channels);
#image = cv.create_image( cv.cvGetSize(imagecv), cv.CV_8U, imagecv.channels);
print image.shape
print 'start'
start = time.time()
for x in range(0, 1):
    imagex = simple_region_growing(imagecv8, seed, threshold)
    print 'done'
end = time.time()
print end - start
print 'end'
#imagex = simple_region_growing(imagecv8, seed, threshold)
#imagen = numpy.asarray(imagex)
imagen = numpy.asarray(imagex[:,:])
